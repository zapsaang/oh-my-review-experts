...
describe("normalizeMemoryFinding", () => {
  it("builds a strict MemoryFinding and resolves packagePath from the primary location", () => {
    ...
    expect(finding).not.toHaveProperty("tags");
    expect(finding).not.toHaveProperty("confidence");
    expect(finding).not.toHaveProperty("sourceFindingId");
    expect(finding.repo).not.toHaveProperty("packageName");
    expect(finding.repo).not.toHaveProperty("packageKind");
  });

  it("uses a placeholder evidence value, sourceMalformed, and acknowledged status when evidence is missing", () => {
  ...
  it("folds recommendation into searchable text without adding a recommendation field", () => {
